...
-- IMAGES
-----------------
defineProperty("background", loadImage("fo_panel.png"))
...
freeTexture{ --курсовой маркер
	image = get(h_marker),
	positionX = function()
		return map(get(hor_dev), -2.5, 2.5, 860, 923)
	end,
	positionY = 562,
	width = 4,
	height = 63,
	visible = function()
		return get(dc_bus) == 1 and get(fd_copilot) == 1
	end,
},
--------------------------------------------------------------------
freeTexture{ --глиссадный маркер
	...
	positionX = 860,
	positionY = function()		
		return map(get(vert_dev), -2.5, 2.5, 623, 560)
	end,
	width = 63,
	height = 4,
	visible = function()
...
freeTexture{ --курсовой маркер 
	image = get(h_marker2),
	positionX = function()
		return map(get(hor_dev), -2.5, 2.5, 846, 922)
	end,
	positionY = 433,
	width = 18,
	height = 23,
...
freeTexture{ --глиссадный маркер 
	...
	positionX = 979,
	positionY = function()		
		return map(get(vert_dev), -2.5, 2.5, 609, 554)
	end,
	width = 23,
	height = 18,
	visible = function()
...
freeTexture{ --скольжение
	image = get(slip_ball),
	positionX = function()
		return map(get(slip), -10, 10, 827, 945)
	end,
	positionY = 463,
	width = 14,
	height = 14,
...
texture{ --FUEL
	...
	position = {55, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
--pk_left
...
texture{ 
	...
	position = {275, 719, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(fuel_tank_selector) == 4 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
--left1_fuel_pump
texture{ 
	...
	position = {100, 586, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(fl_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
--left2_fuel_pump
texture{ 
	image = get(fuel_pump_on),
	position = {148, 586, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(sl_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
--right1_fuel_pump
texture{ 
	image = get(fuel_pump_on),
	position = {400, 586, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(fr_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
--right2_fuel_pump
texture{ 
	image = get(fuel_pump_on),
	position = {1787 - 1339, 586, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(sr_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
...
texture{ 
	image = get(valve_open_h),
	position = {1536 - 1339, 560, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(fm_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
...
texture{ 
	image = get(valve_open_h),
	position = {1684 - 1339, 560, 34, 34},
	visible = function()
		return get(mfi_mnemo_fuel) == 1 and get(sm_fuel_tank) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
tape { --left_tank
        position = {1422 - 1339, 568, 15, 70},
...
texture{ --ELEC
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --gen1_on
	image = get(gen1_on),
	position = {1455 - 1339, 483, 348, 134},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(generator_on_1) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --gpu_on
	image = get(gpu_on),
	position = {1456 - 1339, 483, 348, 134},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(gpu_on_power) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --apu_on
	...
	position = {1456 - 1339, 483, 348, 134},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(generator_on_apu) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --gen2_on
	...
	position = {1456 - 1339, 483, 348, 134},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(generator_on_2) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --rect1_on
	image = get(rect1_on),
	position = {1431 - 1339, 617, 398, 154},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(inverter_on_1) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --rect2_on
	...
	position = {1431 - 1339, 617, 398, 154},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(inverter_on_2) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --rect3_on
	...
	position = {1431 - 1339, 617, 398, 154},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(inverter_on_3) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --bat1_on
	image = get(bat1_on),
	position = {1432 - 1339, 669, 399, 71},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(battery_on_1) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --bat2_on
	image = get(bat2_on),
	position = {1431 - 1339, 669, 399, 71},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(battery_on_2) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --bat3_on
	image = get(bat3_on),
	position = {1430 - 1339, 669, 399, 71},
	visible = function()
		return get(mfi_mnemo_elec) == 1 and get(battery_on_3) == 1 and get(dc_bus)
	end,
},
------------------------------------------------------------------------------------------------------------------------------------------
needle{ 
	image = get(green_sector), --сектор левого двигателя
	position = {1398 - 1339, 654, 108, 108},
	angle = function()
		return get(eng1_temp)
	end,
	visible = function()
		return get(mfi_mnemo_eng) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
needle{ 
	image = get(green_sector), --сектор правого двигателя
	position = {1511 - 1339, 654, 108, 108},
	angle = function()
...
texture{ --AIR
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_air) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --CONF
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_conf) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --HYDR
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_hydr) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
needle{ --TCAS nav
	image = get(nav_circle),
	position = {1394 - 1339, 270, 472, 472},
	angle = function()
		return get(heading) * -1
	end,
	visible = function()
		return get(mfi_mnemo_tcas) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
...
freeTexture{ --tcas_1
	...
	positionX = function()
		if get(tcas1_bearing) < 0 then
			return map(get(tcas1_distance), 50000, 0, 94, 295)
		else
			return map(get(tcas1_distance), 0, 50000, 295, 495)
		end
	end,
	positionY  = function()
		if get(tcas1_bearing) < 0 then
...
freeTexture{ --tcas_2
	...
	positionX = function()
		if get(tcas2_bearing) < 0 then
			return map(get(tcas2_distance), 50000, 0, 94, 295)
		else
			return map(get(tcas2_distance), 0, 50000, 295, 495)
		end
	end,
	positionY  = function()
		if get(tcas2_bearing) < 0 then
...
freeTexture{ --tcas_3
	...
	positionX = function()
		if get(tcas3_bearing) < 0 then
			return map(get(tcas3_distance), 50000, 0, 94, 295)
		else
			return map(get(tcas3_distance), 0, 50000, 295, 495)
		end
	end,
	positionY  = function()
		if get(tcas3_bearing) < 0 then
...
freeTexture{ --tcas_4
	...
	positionX = function()
		if get(tcas4_bearing) < 0 then
			return map(get(tcas4_distance), 50000, 0, 94, 295)
		else
			return map(get(tcas4_distance), 0, 50000, 295, 495)
		end
	end,
	positionY  = function()
		if get(tcas4_bearing) < 0 then
...
texture{ --SYSTEM
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_sys) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --STAT
	...
	position = {1394 - 1339, 162, 472, 621},
	visible = function()
		return get(mfi_mnemo_stat) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
...
texture{ --ПРЕДУПР
	...
	position = {757, 956, 63, 29},
	visible = function()
		return get(master_caution) == 1 and get(dc_bus)
	end,
},
---------------------------------------------------------------------
texture{ --АВАР
	image = get(fault_light),
	position = {910, 956, 63, 29},
	visible = function()
		return get(master_warning) == 1 and get(dc_bus)
	end,
},
------------------------------------------------------------------------------------------------------------------------------------------
